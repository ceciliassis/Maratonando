...
int main(){
  ...
  while(scanf("%d %d", &N, &K) != EOF && N){
    ...
    build(1, 1, N); // sempre constroi do primeiro seguimento, porque ele vai conter todo o intervalo
    for(int i = 0; i < K; i++){
      char c;
      int a, b;
      scanf(" %c %d %d", &c,&a,&b);
      if(c == 'C'){ // change
        // no inicial, limite esquerda, limite direira
        // posicao, valor
        update(1, 1, N , a, b); // update no ponto a e qer colocar o valor b
      }else{
